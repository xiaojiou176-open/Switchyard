...
This internal working pack is the repo-side packing list for Switchyard's next
heavy outward lanes.

Use it when you need one practical answer:
...
Think of it like a shipping checklist. `README.md` is the storefront sign,
while this file is the box label that says what is actually inside before you
send it to a registry or host ecosystem.

## Current lane split

Switchyard has three outward shapes today:
...
That set is the honest registry-facing box. It already explains the name,
version, stdio transport, environment variables, and current not-live boundary.
...
The runtime-diagnostics packet is a different box. It is useful for host
ecosystems that want a self-contained folder, but it is still a secondary
packet lane.
...
[`../mcp/mcp-listings-cockpit.md`](../mcp/mcp-listings-cockpit.md)
...
receipt inside the packet lane. Do not let that leak upward into repo-wide
`listed-live` wording.
...
Those are different cargo families. They should not be stuffed into the current
read-only registry/host packet just to make the box look bigger.
